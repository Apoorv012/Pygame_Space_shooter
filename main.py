...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VEL >= 0:
            yellow.x -= VEL             # LEFT
    if keys_pressed[pygame.K_d] and yellow.x + yellow.height + VEL <= BORDER.x:
            yellow.x += VEL             # RIGHT       
...
def red_handle_movement(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x - VEL >= BORDER.x + BORDER.width:
            red.x -= VEL             # LEFT
    if keys_pressed[pygame.K_RIGHT] and red.x + red.height + VEL <= WIDTH:
            red.x += VEL             # RIGHT       
...
def main():
    ...
    yellow = pygame.Rect(200, 230, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)
    red = pygame.Rect(645, 230, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LSHIFT and len(yellow_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(yellow.x + yellow.height, yellow.y + yellow.width/2 - 2.5, 10, 5)
                    yellow_bullets.append(bullet)
                    BULLET_FIRE_SOUND.play()

                if event.key == pygame.K_RSHIFT and len(red_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(red.x - 10, red.y + red.width/2 - 2.5, 10, 5)
